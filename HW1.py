...
def dynamicProgramMatrixGenerator(seq1,seq2):
    mat =numpy.zeros((len(seq1)+1,len(seq2)+1))
    ...
    for i in range(1,len(seq1)+1):
        for j in range(1,len(seq2)+1):
            leftMove = max(mat [i,j-1] + gapPenalty,0)
            upMove = max(mat [i-1,j] + gapPenalty,0)
            cornerMove = max(mat [i-1,j-1] + PAM250[seq1[i-1]][seq2[j-1]],0)
            
            if ((leftMove >= upMove) and (leftMove >= cornerMove) ):
                mat[i][j]=leftMove
            elif (upMove >= cornerMove):
                mat[i][j]=upMove
            else:
                mat[i][j]=cornerMove
            if (maximumNumber[0]<mat[i][j]):
                maximumNumber[0]=mat[i][j]
                maximumNumber[1]=[i,j]
    
    return mat, maximumNumber

def traceBack(mat, i,j,traceArray):
    global gapPenalty, seq2 , seq1
    
    if (mat[i][j] ==mat[i][j-1] + gapPenalty):
        if (mat[i][j-1] == 0):
            traceArray.append([[i,j-1],"left"])
            return True
        if (traceBack(mat,i,j-1,traceArray) == True):
            traceArray.append([[i,j-1],"left"])
            return True
        
    if (mat[i][j] ==mat[i-1][j] + gapPenalty):
        if (mat[i-1][j] == 0):
            traceArray.append([[i-1,j],"up"])
            return True
        if (traceBack(mat,i-1,j,traceArray) == True):
            traceArray.append([[i-1,j],"up"])
            return True
    
    if (mat[i][j] ==mat[i-1][j-1] + PAM250[seq1[i-1]][seq2[j-1]]):
        if (mat[i-1][j-1] == 0):
            traceArray.append([[i-1,j-1],"corner"])
            return True
        if (traceBack(mat,i-1,j-1,traceArray) == True):
            traceArray.append([[i-1,j-1],"corner"])
            return True
...
def __main__():
    ...
    mat , maximumNumber = dynamicProgramMatrixGenerator(seq1,seq2)
    # print(mat)
    
    print (int(maximumNumber[0]))

    traceArray = []

    traceBack(mat,maximumNumber[1][0],maximumNumber[1][1], traceArray)
    # print(traceArray)

    seq1Output = "" 
    seq2Output = "" 
    
    for element in traceArray:
        # print(element)
        if (element[1] =='corner'):
            seq1Output += seq1[element[0][0]]
            seq2Output += seq2[element[0][1]]
        elif (element[1] =='left'):
            seq1Output += '-'
            seq2Output += seq2[element[0][1]]
        elif (element[1] =='up'):
